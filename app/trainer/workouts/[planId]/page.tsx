...
export default async function WorkoutPlanPage({ params }: PageProps) {
  ...
  if (!plan) notFound();
  return <div className="trainer-page"><header className="trainer-page__heading trainer-page__heading--compact"><div><Link className="trainer-back" href="/trainer/workouts"><Icon name="chevron" size={14} /> Workout plans</Link><h1>{plan.title}</h1><p>Build the day-by-day structure, then publish an immutable version.</p></div></header><WorkoutPlanEditor gyms={gyms.map((item: any) => item.gym)} plan={{ id: plan.id, title: plan.title, description: plan.description, gymId: plan.gymId, isTemplate: plan.isTemplate, status: plan.status, version: plan.version, assignmentCount: plan._count.assignments, days: plan.days.map((day: any) => ({ key: day.id, name: day.name, notes: day.notes || "", exercises: day.exercises.map((exercise: any) => ({ key: exercise.id, name: exercise.name, sets: exercise.sets, reps: exercise.reps || "", weight: exercise.weight || "", tempo: exercise.tempo || "", restSeconds: exercise.restSeconds, durationSeconds: exercise.durationSeconds, distanceMeters: exercise.distanceMeters, notes: exercise.notes || "" })) })) }} /><AssignmentPanel kind="workouts" planId={plan.id} published={plan.status === "ACTIVE"} clients={clients.map((client: any) => ({ id: client.id, name: client.user.name }))} assignments={plan.assignments.filter((item: any) => item.trainerClient).map((item: any) => ({ id: item.id, status: item.status, assignedAt: item.assignedAt.toISOString(), name: item.trainerClient!.user.name, email: item.trainerClient!.user.email }))} /></div>;
}
