...
export default async function NutritionPlanPage({ params }: PageProps) {
  ...
  if (!plan) notFound();
  return <div className="trainer-page"><header className="trainer-page__heading trainer-page__heading--compact"><div><Link className="trainer-back" href="/trainer/nutrition"><Icon name="chevron" size={14} /> برنامه‌های تغذیه</Link><h1>{plan.title}</h1><p>وعده‌ها، مقدارها، کالری و اهداف درشت‌مغذی این نسخه را مشخص کنید.</p></div></header><NutritionPlanEditor gyms={gyms.map((item: any) => item.gym)} plan={{ id: plan.id, title: plan.title, description: plan.description, dietaryRestrictions: plan.dietaryRestrictions, dailyCalories: plan.dailyCalories, gymId: plan.gymId, isTemplate: plan.isTemplate, status: plan.status, version: plan.version, assignmentCount: plan._count.assignments, days: plan.days.map((day: any) => ({ key: day.id, name: day.name, notes: day.notes || "", targetCalories: day.targetCalories, targetProtein: day.targetProtein, targetCarbs: day.targetCarbs, targetFat: day.targetFat, meals: day.meals.map((meal: any) => ({ key: meal.id, name: meal.name, scheduledTime: meal.scheduledTime || "", notes: meal.notes || "", foodItems: meal.foodItems.map((food: any) => ({ key: food.id, name: food.name, quantity: food.quantity, unit: food.unit || "", calories: food.calories, protein: food.protein, carbs: food.carbs, fat: food.fat, notes: food.notes || "" })) })) })) }} /><AssignmentPanel kind="nutrition" planId={plan.id} published={plan.status === "ACTIVE"} clients={clients.map((client: any) => ({ id: client.id, name: client.user.name }))} assignments={plan.assignments.filter((item: any) => item.trainerClient).map((item: any) => ({ id: item.id, status: item.status, assignedAt: item.assignedAt.toISOString(), name: item.trainerClient!.user.name, email: item.trainerClient!.user.email }))} /></div>;
}
