...
function date(value: Date | null) { return value ? new Intl.DateTimeFormat("fa-IR", { month: "short", day: "numeric", year: "numeric" }).format(value) : "شروع نشده"; }
...
export default async function TrainerStudentsPage() {
  ...
  return <div className="trainer-page"><header className="trainer-page__heading trainer-page__heading--compact"><div><span>شاگردان شما</span><h1>شاگردان</h1><p>رابطه‌های مربی‌گری را مدیریت کنید و هر مراجع را رو به جلو نگه دارید.</p></div><InviteStudent gyms={gyms.map((item: any) => item.gym)} /></header><section className="trainer-summary"><div><span>شاگردان فعال</span><strong>{counts.active}</strong></div><div><span>درخواست‌های در انتظار</span><strong>{counts.pending}</strong></div><div><span>کم‌فعال (بیش از ۷ روز)</span><strong>{lowActive.size}</strong></div><div><span>در انتظار پاسخ</span><strong>{unanswered.length}</strong></div></section>
  ...
  {clients.length ? <div className="trainer-student-grid">{clients.map((client: any) => <article key={client.id} className={lowActive.has(client.userId) ? "is-low-active" : ""}><div className="trainer-student-top"><span className="trainer-avatar">{client.user.name.slice(0, 2).toUpperCase()}</span><b className={`trainer-status trainer-status--${client.status.toLowerCase()}`}>{faStatus(client.status)}</b></div><h2>{client.user.name}</h2><p>{client.user.email}{client.user.phone ? ` · ${client.user.phone}` : ""}</p>{lowActive.has(client.userId) && <p className="trainer-low-active">⚠ کم‌فعال — آخرین تمرین: {lastByUser.get(client.userId)?.toLocaleDateString("fa-IR") || "ثبت نشده"}</p>}<div className="trainer-student-context"><Icon name="location" size={13} /> {client.gym?.name || "مربی‌گری مستقل"}<span>از {date(client.startedAt)}</span></div><div className="trainer-student-stats"><span><strong>{client._count.workoutAssignments}</strong><small>تمرین‌ها</small></span><span><strong>{client._count.dietAssignments}</strong><small>برنامه‌های غذایی</small></span><span><strong>{client._count.sessions}</strong><small>جلسات</small></span></div><div className="trainer-student-footer"><Link href={`/trainer/schedule?student=${client.id}`}>زمان‌بندی جلسه <Icon name="arrow" size={13} /></Link><StudentStatusActions clientId={client.id} status={client.status} />{client.status === "ACTIVE" && lowActive.has(client.userId) && <NudgeButton clientId={client.id} />}</div></article>)}</div> : <div className="trainer-panel trainer-empty"><Icon name="users" size={26} /><strong>هنوز شاگردی ثبت نشده است</strong><span>عضوی را دعوت کنید یا منتظر درخواست مربی‌گری بمانید.</span></div>}</div>;
}
