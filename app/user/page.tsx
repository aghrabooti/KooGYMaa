...
export default async function MemberOverviewPage() {
  ...
  const metrics: Array<{ icon: IconName; label: string; value: string; note: string; tone: string }> = [
    { icon: "dumbbell", label: "تداوم تمرین", value: `${workoutRate}%`, note: `${workoutLogs.filter((log: any) => log.status === "COMPLETED").length} workouts (60d)`, tone: "lime" },
    { icon: "heart", label: "تداوم تغذیه", value: `${nutritionRate}%`, note: `${nutritionLogs.filter((log: any) => log.completedAt).length} complete روز`, tone: "orange" },
    { icon: "flame", label: "رشته تداوم", value: `${streak} روز`, note: streak > 0 ? "تداوم را حفظ کنید" : "برای شروع یک تمرین را کامل کنید", tone: "violet" },
    { icon: "trend", label: "آخرین وزن", value: measurement?.weightKg ? `${measurement.weightKg} کیلوگرم` : "—", note: measurement ? `Recorded ${measurement.recordedAt.toLocaleDateString("fa-IR")}` : "ثبت حضور", tone: "blue" },
  ];
  ...
  if (session) actions.push({ label: `جلسه بعدی: ${session.title}، ساعت ${session.startsAt.toLocaleTimeString("en", { hour: "numeric", minute: "2-digit" })}`, href: "/user/schedule" });
  if (!workout) actions.push({ label: "هنوز برنامه تمرینی تخصیص داده نشده است — از مربی‌تان بخواهید", href: "/user/trainers" });

  return (
    <div className="member-page">
      <header className="member-page__heading"><div><span>روز شما</span><h1>Welcome back, {user.name.split(" ")[0]}.</h1><p>تمرین، تغذیه، مربی‌گری و عضویت‌های شما در یک نگاه.</p></div><Link className="member-primary-button" href="/user/workouts">شروع تمرین <Icon name="arrow" size={15} /></Link></header>

      {doneSteps < steps.length && (
        <section className="member-panel member-onboarding" aria-label="راهنمای شروع">
          <div className="member-panel__heading"><div><h2>Getting started ({doneSteps}/{steps.length})</h2><p>پنج قدم سریع برای باز شدن همه امکانات</p></div></div>
      ...
      {notifications.length > 0 && <section className="member-panel member-dashboard-notifications"><div className="member-panel__heading"><div><h2>خوانده‌نشده‌ها</h2><p>{notifications.length} recent notifications</p></div><Link href="/user/notifications">باز کردن صندوق</Link></div><div>{notifications.map((item: any) => <Link href={item.href || "/user/notifications"} key={item.id}><span><Icon name="bell" size={15} /></span><div><strong>{item.title}</strong><small>{item.message}</small></div><Icon name="chevron" size={14} /></Link>)}</div></section>}
    </div>
  );
}
