...
export default async function MemberOverviewPage() {
  ...
  const metrics: Array<{ icon: IconName; label: string; value: string; note: string; tone: string }> = [
    { icon: "dumbbell", label: "تداوم تمرین", value: `${workoutRate}%`, note: `${workoutLogs.filter((log: any) => log.status === "COMPLETED").length} تمرین (۶۰ روز)`, tone: "lime" },
    { icon: "heart", label: "تداوم تغذیه", value: `${nutritionRate}%`, note: `${nutritionLogs.filter((log: any) => log.completedAt).length} روز کامل`, tone: "orange" },
    { icon: "flame", label: "رشته تداوم", value: `${streak} روز`, note: streak > 0 ? "تداوم را حفظ کنید" : "برای شروع یک تمرین را کامل کنید", tone: "violet" },
    { icon: "trend", label: "آخرین وزن", value: measurement?.weightKg ? `${measurement.weightKg} کیلوگرم` : "—", note: measurement ? `ثبت‌شده: ${measurement.recordedAt.toLocaleDateString("fa-IR")}` : "ثبت حضور", tone: "blue" },
  ];

  // Item 7: onboarding guide + required actions.
  const steps = [
    { done: Boolean(gym), label: "عضو باشگاه شوید", href: "/user/gyms" },
    { done: Boolean(subscriptions.length), label: "فعال‌سازی اشتراک", href: "/user/subscriptions" },
    { done: Boolean(trainer), label: "با یک مربی ارتباط بگیرید", href: "/user/trainers" },
    { done: Boolean(workout), label: "برنامه تمرینی‌تان را دریافت کنید", href: "/user/workouts" },
    { done: Boolean(measurement), label: "نخستین حضور خود را ثبت کنید", href: "/user/progress" },
  ];
  const doneSteps = steps.filter((s) => s.done).length;
  const actions: Array<{ label: string; href: string }> = [];
  if (pendingMembership) actions.push({ label: `عضویت در ${pendingMembership.gym.name} در انتظار بررسی است`, href: "/user/gyms" });
  if (unfinished > 0) actions.push({ label: `${unfinished} تمرین ناتمام — از همان‌جا ادامه بده`, href: "/user/workouts" });
  if (nearest) {
    const d = daysUntil(nearest.endDate, now);
    if (d <= 7) actions.push({ label: `«${nearest.plan.name}» ${d} روز دیگر منقضی می‌شود — به‌موقع تمدید کنید`, href: "/user/subscriptions" });
  }
  if (session) actions.push({ label: `جلسه بعدی: ${session.title}، ساعت ${session.startsAt.toLocaleTimeString("fa-IR", { hour: "numeric", minute: "2-digit" })}`, href: "/user/schedule" });
  if (!workout) actions.push({ label: "هنوز برنامه تمرینی تخصیص داده نشده است — از مربی‌تان بخواهید", href: "/user/trainers" });

  return (
    <div className="member-page">
      <header className="member-page__heading"><div><span>روز شما</span><h1>{user.name.split(" ")[0]}، خوش برگشتی.</h1><p>تمرین، تغذیه، مربی‌گری و عضویت‌های شما در یک نگاه.</p></div><Link className="member-primary-button" href="/user/workouts">شروع تمرین <Icon name="arrow" size={15} /></Link></header>

      {doneSteps < steps.length && (
        <section className="member-panel member-onboarding" aria-label="راهنمای شروع">
          <div className="member-panel__heading"><div><h2>شروع ({doneSteps}/{steps.length})</h2><p>پنج قدم سریع برای باز شدن همه امکانات</p></div></div>
      ...
      {(todayDay || session || actions.length > 0) && (
        <section className="member-today-strip">
          ...
          {session && <Link className="member-today-card" href="/user/schedule"><span><Icon name="calendar" size={19} /></span><div><small>جلسه بعدی · {sessions.length} زمان‌بندی‌شده</small><strong>{session.title}</strong></div><Icon name="chevron" size={14} /></Link>}
      ...
      <div className="member-dashboard-grid"><section className="member-panel member-today"><div className="member-panel__heading"><div><h2>سرویس‌های فعال شما</h2><p>هرچه هم‌اکنون در دسترس شماست</p></div></div><div><Link href="/user/workouts"><span><Icon name="dumbbell" size={19} /></span><div><strong>{workout?.plan.title || "برنامه تمرینی تخصیص داده نشده است"}</strong><small>{workout ? `${workout.plan.days.length} روز تمرینی` : "با یک مربی ارتباط بگیرید"}</small></div><Icon name="chevron" size={14} /></Link><Link href="/user/nutrition"><span><Icon name="heart" size={19} /></span><div><strong>{nutrition?.plan.title || "برنامه تغذیه‌ای وجود ندارد"}</strong><small>{nutrition?.plan.dailyCalories ? `${nutrition.plan.dailyCalories} هدف کالری` : "راهنمای تغذیه منعطف"}</small></div><Icon name="chevron" size={14} /></Link><Link href="/user/gyms"><span><Icon name="building" size={19} /></span><div><strong>{gym?.gym.name || "یک باشگاه پیدا کنید"}</strong><small>{gym ? gym.gym.city || "عضویت فعال" : "مشاهده باشگاه‌های نزدیک"}</small></div><Icon name="chevron" size={14} /></Link><Link href="/user/trainers"><span><Icon name="users" size={19} /></span><div><strong>{trainer?.trainer.user.name || "یک مربی پیدا کنید"}</strong><small>{trainer?.trainer.specialty || "مربی‌گری شخصی"}</small></div><Icon name="chevron" size={14} /></Link></div></section><section className="member-panel member-next-session"><div className="member-panel__heading"><div><h2>جلسه بعدی</h2><p>نوبت مربی‌گری آینده شما</p></div></div>{session ? <div><span><Icon name="calendar" size={22} /></span><small>{session.startsAt.toLocaleDateString("fa-IR", { weekday: "long", month: "short", day: "numeric" }).toUpperCase()}</small><strong>{session.startsAt.toLocaleTimeString("fa-IR", { hour: "numeric", minute: "2-digit" })}</strong><h3>{session.title}</h3><p>{session.trainer.user.name} · {session.gym?.name || "خصوصی / آنلاین"}</p><Link href="/user/schedule">مشاهده برنامه <Icon name="arrow" size={14} /></Link></div> : <div className="member-empty"><Icon name="calendar" size={24} /><h2>جلسه آینده‌ای وجود ندارد</h2><p>برنامه شما خالی است.</p></div>}</section></div>
      <div className="member-dashboard-bottom"><section className="member-panel"><div className="member-panel__heading"><div><h2>آخرین بازخورد مربی</h2><p>راهنمایی مربی شما</p></div><Link href="/user/progress">مشاهده همه</Link></div>{feedback ? <blockquote><Icon name="sparkles" size={18} /><p>{feedback.content}</p><footer>{feedback.author.name} · {feedback.createdAt.toLocaleDateString("fa-IR")}</footer></blockquote> : <p className="member-empty-line">هنوز بازخوردی ثبت نشده.</p>}</section><section className="member-panel"><div className="member-panel__heading"><div><h2>عضویت</h2><p>دسترسی اشتراک فعلی</p></div><Link href="/user/subscriptions">جزئیات</Link></div>{nearest ? <div className="member-subscription-mini"><span><Icon name="credit-card" size={19} /></span><div><strong>{nearest.plan.name}</strong><small>{nearest.gym.name}</small></div><b>{daysUntil(nearest.endDate, now)} days left</b></div> : <p className="member-empty-line">اشتراک فعالی وجود ندارد.</p>}</section></div>
      {notifications.length > 0 && <section className="member-panel member-dashboard-notifications"><div className="member-panel__heading"><div><h2>خوانده‌نشده‌ها</h2><p>{notifications.length} اعلان اخیر</p></div><Link href="/user/notifications">باز کردن صندوق</Link></div><div>{notifications.map((item: any) => <Link href={item.href || "/user/notifications"} key={item.id}><span><Icon name="bell" size={15} /></span><div><strong>{item.title}</strong><small>{item.message}</small></div><Icon name="chevron" size={14} /></Link>)}</div></section>}
    </div>
  );
}
