...
export async function POST(request: NextRequest, { params }: Context) {
  ...
  await prisma.$transaction(async (transaction: any) => {
    ...
    const cancelledAt = now.toLocaleDateString("fa-IR");
  return NextResponse.json({ message: `اشتراک لغو شد (${cancelledAt}). دسترسی باشگاه قطع و پرداخت‌های در انتظار لغو شدند.`, cancelledAt: now });
}
