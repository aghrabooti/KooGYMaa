...
export async function POST(request: NextRequest, { params }: Context) {
  ...
  const [plan, client] = await Promise.all([
    prisma.workoutPlan.findFirst({ where: { id: planId, trainerId: authorization.access.profile.id, status: "ACTIVE" }, select: { id: true, title: true } }),
  ...
  const assignment = await prisma.$transaction(async (transaction) => {
    ...
    const created = await transaction.workoutAssignment.upsert({
    ...
    await transaction.notification.create({ data: { userId: client.userId, type: "PLAN_ASSIGNED", title: "New workout plan", message: `${plan.title} is ready for you.`, href: "/user/workouts" } });
    return created;
  });
