...
import { validateWorkoutPlan } from "@/lib/plan-validation";
...
export async function GET(request: NextRequest) {
  ...
  const plans = await prisma.workoutPlan.findMany({
    ...
    select: {
      id: true, familyId: true, title: true, description: true, status: true, version: true,
      isTemplate: true, publishedAt: true, updatedAt: true,
...
export async function POST(request: NextRequest) {
  ...
  if (validation.data.gymId) {
    const membership = await prisma.gymTrainer.findUnique({
  ...
  const plan = await prisma.workoutPlan.create({
    data: {
      ...
      isTemplate: validation.data.isTemplate,
      status: "DRAFT",
      days: {
        create: validation.data.days.map((day, dayIndex) => ({
          dayNumber: dayIndex + 1,
          name: day.name,
          notes: day.notes,
          exercises: { create: day.exercises.map((exercise, exerciseIndex) => ({ ...exercise, order: exerciseIndex + 1 })) },
        })),
      },
    },
