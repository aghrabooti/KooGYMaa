...
export function WorkoutPlanEditor({ gyms, plan }: { gyms: Array<{ id: string; name: string }>; plan: Plan }) {
  ...
  async function save(): Promise<boolean> {
    ...
    try {
      ...
      if (!response.ok) { setError(data.error || "Unable to save plan."); return false; }
      setMessage("Workout plan saved.");
      router.refresh();
      return true;
    } catch { setError("Unable to connect."); return false; } finally { setPending(""); }
  }
  ...
  async function publish() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) { setError(data.error || "Unable to publish plan."); return; }
      router.refresh();
    } finally { setPending(""); }
  }

  return <section className="trainer-plan-editor">
    <div className="trainer-plan-editor__meta"><div><label><span>Plan title *</span><input disabled={!editable} value={title} onChange={(event) => setTitle(event.target.value)} /></label><label><span>Gym context</span><select disabled={!editable} value={gymId} onChange={(event) => setGymId(event.target.value)}><option value="">Independent coaching</option>{gyms.map((gym) => <option key={gym.id} value={gym.id}>{gym.name}</option>)}</select></label><label className="trainer-form-wide"><span>Description</span><textarea disabled={!editable} rows={3} value={description} onChange={(event) => setDescription(event.target.value)} /></label><label className="trainer-check"><input checked={isTemplate} disabled={!editable} type="checkbox" onChange={(event) => setTemplate(event.target.checked)} /><span>Reusable template</span></label></div><aside><span className={`trainer-status trainer-status--${plan.status.toLowerCase()}`}>{plan.status}</span><strong>Version {plan.version}</strong><small>{plan.assignmentCount} assignments</small>{dirty && editable && <small className="trainer-dirty">● تغییرات ذخیره‌نشده</small>}</aside></div>
    {!editable && <p className="trainer-plan-notice"><Icon name="lock" size={15} /> Published or assigned versions are read-only. Create a new version to make changes.</p>}
    <div className="trainer-workout-days">
      {days.map((day, dayIndex) => (
        <article className="trainer-workout-day" key={day.key}>
          <header><span>{dayIndex + 1}</span><input disabled={!editable} value={day.name} onChange={(event) => patchDay(dayIndex, { name: event.target.value })} placeholder="Training day name *" /><input disabled={!editable} value={day.notes} onChange={(event) => patchDay(dayIndex, { notes: event.target.value })} placeholder="Day notes" />
            {editable && <span className="trainer-day-tools">
              ...
              <button aria-label="Remove day" onClick={() => setDays((items) => items.filter((_, i) => i !== dayIndex))}>×</button>
            </span>}
          </header>
          <div className="trainer-exercise-head"><span>Exercise *</span><span>Sets</span><span>Reps</span><span>Weight</span><span>Tempo</span><span>Rest</span><span /></div>
          ...
          {editable && <button className="trainer-add-row" onClick={() => patchDay(dayIndex, { exercises: [...day.exercises, blankExercise()] })}><Icon name="plus" size={14} /> Add exercise</button>}
      ...
      {editable && <button className="trainer-add-day" onClick={() => setDays((items) => [...items, { key: crypto.randomUUID(), name: `Day ${items.length + 1}`, notes: "", exercises: [blankExercise()] }])}><Icon name="plus" size={15} /> Add training day</button>}
    </div>
