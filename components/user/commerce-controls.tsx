...
export function ReviewEditor({ existing, targetId, type, verified }: { existing?: { score: number; comment: string | null }; targetId: string; type: "gym" | "trainer"; verified: boolean }) {
  ...
  if(!verified&&!existing)return <p className="review-eligibility"><Icon name="shield" size={15}/> برای ثبت دیدگاه، {type === "gym" ? "یک عضویت تأییدشده" : "یک رابطه مربی‌گری تأییدشده"} لازم است.</p>;
